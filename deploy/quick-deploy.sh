...
if docker ps --format '{{.Names}}' | grep -q "fractal-trader-production"; then
    ...
    echo -e "  ${BLUE}Bot status:${NC}     docker exec -it fractal-trader-production python -m live.cli status"
    echo -e "  ${BLUE}Performance:${NC}    docker exec -it fractal-trader-production python -m live.cli report"
